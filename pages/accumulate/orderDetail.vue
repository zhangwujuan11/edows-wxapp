<template>
	<view>
		...
		<view class="address-section  logistics-section" v-if="orderInfo.wlNum">
			...
			<view class="wliu-pic">
				<image v-if="orderInfo.fhpz" :src="orderInfo.fhpz" mode="widthFix"></image>
			</view>
		</view>
		...
		<view class="goods-section">
			...
			<view class="g-item" @click='navToDetailPage(orderInfo)'>
				<image :src="orderInfo.masterImg"></image>
				<view class="right">
					<text class="title clamp">{{orderInfo.goodsTitle}}</text>
					<view class="price-box">
						<text class="price"> {{orderInfo.scoreNumber}}</text>
						<text class="number">x {{orderInfo.totalSum}}</text>
					</view>
				</view>
			</view>
			<!-- </navigator> -->
		...
		<view class="facture-setion">
			...
			<view class="f-item">
				...
				<text class="red">{{orderInfo.totalScoreNumber}}积分</text>
			</view>
	...
	export default {
		...
		methods: {
			...
			async getDatas() {
				...
				let res = await this.$axios(this.$baseUrl.scoreGoodsOrderDetail, params);
					...
					success: async (loginRes) => {
						...
						uni.requestPayment({
							...
							success: function(res) {
								uni.redirectTo({
									url: '/pages/money/paySuccess?totalPrice=' + item
										.totalPrice
								})
							},
							fail: function(err) {
