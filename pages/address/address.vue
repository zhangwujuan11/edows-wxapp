...
<script>
	...
	export default {
		...
		async onLoad(option) {
			...
			this.orderData= option.orderData ? option.orderData : '';
		...
		methods: {
			...
			addAddress(type, item) {
				uni.navigateTo({
					url: `/pages/address/addressManage?type=${type}&source=${this.source}&data=${JSON.stringify(item)}&orderData=${this.orderData}`
				})
			},
			//添加或修改成功之后回调
